function crfaa_20070326adpDTpractice(observer,adapter)
%  crfaa_20070326adpDTpractice.m
%
%        $Id: crfaa_MMDDYY.m
...
%% switch this to 0 if this is a real run to 1 if it is just for testing the code
istest=1;
%% 
% check arguments
if ~any(nargin == [2])
    help  crfaa_20070326adpDTpractice
    return
end
...
if (~status && (strcmp(whichcomputer,'jackson.cns.nyu.edu') || strcmp(whichcomputer,'amacbookpro-2.local') ... 
                                                            || strcmp(whichcomputer,'stimulus-g5')))
    if strcmp(whichcomputer,'jackson.cns.nyu.edu')
        datafoldername = 'data_psychophysics';
        psychoORscanner = 1;
        disp('[crfaa_20070326adpDTpractice] ... running psychophysics on jackson.');
        disp('[crfaa_20070326adpDTpractice] cd to /private/var/automount/i/7.2/frakkopesto/fmritools/ATT_ADPT_FILES/CRFAA/')
    ...
    elseif strcmp(whichcomputer,'stimulus-g5')
        ...
        psychoORscanner = 2;
        disp('[crfaa_20070326adpDTpractice] ... running a scan ON stimulus-g5 with the projector.');
        disp('[crfaa_20070326adpDTpractice] cd to /CECHCKSCANENRDIRES/ATT_ADPT_FILES/CRFAA/')
        cd('/CECHCKSCANENRDIRES/ATT_ADPT_FILES/CRFAA/')

    elseif strcmp(whichcomputer,'amacbookpro-2.local')
        datafoldername = 'data_psychophysics';
        psychoORscanner = 1;
        disp('[crfaa_20070326adpDTpractice] ... running laptopt for test purposes.');
        disp('[crfaa_20070326adpDTpractice] cd to /Users/frakkopesto/data/nyu/attadp/data_psychophysics/')
        cd('/Users/frakkopesto/data/nyu/attadp/')

    else
        % only two data folder names allowed:
        disp(sprintf('[crfaa_20070326adpDTpractice] Please choose datafoldername for psychophysics (data_psychophysics) OR for the scanner (data_scanner). %s', datafoldername));
        help  crfaa_20070326adpDTpractice
        return
    end

else
    disp(sprintf('[crfaa_20070326adpDTpractice] command "system" returned status "%i" on the call: "[status,whichcomputer] = system(''hostname'');"',status));
    disp(sprintf('[crfaa_20070326adpDTpractice] command "system" returned computer "%s" on the call: "[status,whichcomputer] = system(''hostname'');"',whichcomputer));
    disp('[crfaa_20070326adpDTpractice] exiting if status ~= 0 (system not checked) OR if whichcomputer not "jackson.cns.nyu.edu" OR "stimulus-g5"');
    return
end
...
if ~isdir(datadirname)
    disp(sprintf('[crfaa_20070326adpDTpractice] Data dir "%s" not found, making data directory.', datadirname));
    mkdir(datadirname);
else
    disp(sprintf('[crfaa_20070326adpDTpractice] Found data directory "%s".', datadirname));
end

% make an observer directory if necessary
datadirname = fullfile(datadirname,observer);
if ~isdir(datadirname);
    disp(sprintf('[crfaa_20070326adpDTpractice] Observer dir "%s" not found, making observer directory.', datadirname));
    mkdir(datadirname);
else
    disp(sprintf('[crfaa_20070326adpDTpractice] Found observer directory "%s".', datadirname));
...
if ~isempty(stimfileindatadir) % there is a stimfile
    ...
    disp(sprintf('[crfaa_20070326adpDTpractice] Previous quest data found, loading quest parameters from: %s',stimfileindatadir(end).name));
    disp(sprintf('[crfaa_20070326adpDTpractice] ... saving data in: %s',datadirname));
else % there is no file yet
    disp('[crfaa_20070326adpDTpractice] No quest data found, will initialize a new quest.');
    disp(sprintf('[crfaa_20070326adpDTpractice] ... saving data in: %s',datadirname));
end
...
if psychoORscanner == 1
    ...
    if isfile(calibrationfile)
        ...
        disp(sprintf('[crfaa_20070326adpDTpractice] found suggested calibration file (%s)',calibrationfile));
    else
        disp(sprintf('[crfaa_20070326adpDTpractice] suggested calibration file NOT found (%s)',calibrationfile));
        return
    end
else
...
%% saving some useull stuff:
myscreen.allowpause = 0;
myscreen.saveData = 1;
myscreen.datadir = datadirname;
myscreen.whichComputer = whichcomputer;
myscreen.whichFunction = which('crfaa_20070326adpDTpractice');
...
task{2}.numTrials = 80;
task{2}.segmin =      [2 .4 .6 .2 .6 .4 1.3 4.5];
task{2}.segmax =      [2 .4 .6 .2 .6 .4 1.3 5.5];
task{2}.segquant =    [ 0  0  0  0  0  0  0  .5];
% wait for back tick
if psychoORscanner
...
stimulus.distractorContrast = {[0 .0175 .035]};
...
% changing stimulus coordinates from dva to pix
global MGL;
xDeg2pix = MGL.xDeviceToPixels;
yDeg2pix = MGL.yDeviceToPixels;
widthPixels = round(stimulus.width*xDeg2pix)+1;
heightPixels = round(stimulus.height*yDeg2pix)+1;

% calculate a grating and gaussian envelope (gaussian is in the alpha channel)
for thisPhase = 1:length(stimulus.phase)
    gratingMatrix{thisPhase} = ones(widthPixels,heightPixels);
