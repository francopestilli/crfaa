function results = crfaa_20070207mq_r(observer,focaldistributed,doadaptation,adaptationC)
%  crfaa_20070207mq_r.m
%
%        $Id: crfaa_MMDDYYmq_r.m
%      usage: crfaa_YYYYMMDDmq_r(observer)
%         by: franco pestilli
%       date: 02/06/2007
%    purpose: measure attended OR distributed TvC functions at
...
%
% this version is to test the hypothesis that if observers can optimize K
% by blocking distributed or focal cue trials the effect f attention should
% be reduced by 2.
%
% ---- help end ----
...
if ~status && (strcmp(deblank(whichcomputer),'grustim.bnf.brain.riken.jp'))
 if strcmp(whichcomputer,'grustim.bnf.brain.riken.jp')
  psychoORscanner = 1;
  disp('[crfaa_20070207mq_r] ... running psychophysics on grustim.');
  if doadaptation
   datafoldername = 'adaptation';
   disp('[crfaa_20070207mq_r] cd to /Users/frakkopesto/data/crfaa/')
   cd('/Users/frakkopesto/data/crfaa/');
  else
   datafoldername = '/Users/frakkopesto/data/effectsize';
   disp('[crfaa_20070207mq_r] cd to /Users/frakkopesto/data/effectsize')
   cd('/Users/frakkopesto/data/effectsize');
  end
 else
  % only two data folder names allowed:
  disp(sprintf('[crfaa_20070207mq_r] Please choose datafoldername for psychophysics (data_psychophysics) OR for the scanner (data_scanner). %s', datafoldername));
  help  crfaa_20070207mq_r
  return
 end
 
else
 disp(sprintf('[crfaa_20070207mq_r] command "system" returned status "%i" on the call: "[status,whichcomputer] = system(''hostname'');"\n',status));
 disp(sprintf('[crfaa_20070207mq_r] command "system" returned computer "%s" on the call: "[status,whichcomputer] = system(''hostname'');"\n',whichcomputer));
 disp('[crfaa_20070207mq_r] exiting if status ~= 0 (system not checked) OR if whichcomputer not "jackson.cns.nyu.edu"');
 return
end
...
if ~isdir(datadirname)
 disp(sprintf('[crfaa_20070207mq_r] Data dir "%s" not found, making data directory.', datadirname));
 mkdir(datadirname);
else
 disp(sprintf('[crfaa_20070207mq_r] Found data directory "%s".', datadirname));
end

% make an observer directory if necessary
observer = sprintf('%s_A%i',observer,adaptationC);
datadirname = fullfile(datadirname,observer);
if ~isdir(datadirname);
 disp(sprintf('[crfaa_20070207mq_r] Observer dir "%s" not found, making observer directory.', datadirname));
 mkdir(datadirname);
else
 disp(sprintf('[crfaa_20070207mq_r] Found observer directory "%s".', datadirname));
...
if ~isempty(stimfileindatadir) % there is a stimfile
 ...
 disp(sprintf('[crfaa_20070207mq_r] Previous quest data found, loading quest parameters from: %s',stimfileindatadir(end).name));
 disp(sprintf('[crfaa_20070207mq_r] ... saving data in: %s',datadirname));
else % there is no file yet
 disp('[crfaa_20070207mq_r] No quest data found, will initialize a new quest.');
 disp(sprintf('[crfaa_20070207mq_r] ... saving data in: %s',datadirname));
end
...
myscreen.whichFunction = which('crfaa_20070207mq_r');
myscreen.stimfileindatadir = datadirname;
myscreen.calibrationFile = 'latest grustim';
...
if doadaptation
 stimulus.doadaptation = doadaptation;
 stimulus.pedestals = [0 .0022 .0044 .0088 .0175 .035 .07 .14];
 stimulus.distractorContrast = {[0 .0175 .035] [0 0.0022 0.0044] [0 0.0044 0.0088] [0 0.0088 0.0175] [0 .0175 .035] [.0175 .035 .07] [.035 .07 .14] [ .07 .14 .28]};
 stimulus.maxContrast = [.08 .08 .08 .1 .1  .1 .18 .36];
 if  adaptationC == 0
  adaptationC = .000001;
  task{1}.seglen = [2 .7];
 else
  adaptationC = adaptationC/100;
  task{1}.seglen = [2 70];
 end
 stimulus.adaptationContrast = adaptationC;
...
% real experiment:
task{2}.focalDistributed = focaldistributed; % 1 = only focal trials, 2 = only distributed trials
task{2}.segmin =      [.8 .4 .6 .2 .6 .4 1.2 1.2];
task{2}.segmax =      [.8 .4 .6 .2 .6 .4 1.2 4.8];
task{2}.segquant =    [ 0  0  0  0  0  0  0  1.2];
% wait for back tick
if psychoORscanner
...
task{2}.parameter.PedContrast = 1:length(stimulus.pedestals);
...
stimulus.focaldistributed = focaldistributed;
...
if ~isfield(stimulus,'quest')
 ...
 if doadaptation
  stimulus.quest.tGuess     = log10([.01 .01 .01 .01 .01 .01 .01 .01; .01 .01 .01 .01 .01 .01 .01 .01]);
 else
  stimulus.quest.tGuess     = log10([.02 .01 .02 .04 .06 .08 .1 .15; .02 .01 .02 .04 .06 .08 .1 .15]);
 end
...
if (task.thistrial.thisseg == 1) % precueing
 ...
 % THIS IS NEW:
 % HERE I SET the CUE CONDITIONS, now 1 and 2 are the same cue condition
 % (two independent staircases each)
 %
 % choose attentional condition for precueing:
 if task.focalDistributed == 1 % 1 = focal 2 = distributed 3 = both
  stimulus.tmp.precueColor{task.thistrial.targetLocation} = stimulus.white;
  
 elseif task.focalDistributed == 2 % 1 = focal 2 = distributed
  stimulus.tmp.precueColor={stimulus.white stimulus.white stimulus.white stimulus.white};
  
 elseif task.focalDistributed == 3 % 1 = focal 2 = distributed 3 = both
...
elseif (task.thistrial.thisseg == 2) % Delay1 (NO top-up)
 ...
 % this draw is done to clean the back-buffer of the screen otherwise high
 % contrast gabors show up
 phaseNum = mod(floor(2*mglGetSecs(task.thistrial.trialstart)/stimulus.flickerPeriod),2)+1;
 drawGabor(.00000001,stimulus.eccentricity*[stimulus.locations{stimulus.tmp.stimLocationindx(1)}],phaseNum);
 drawGabor(.00000001,stimulus.eccentricity*[stimulus.locations{stimulus.tmp.stimLocationindx(2)}],phaseNum);
 drawGabor(.00000001,stimulus.eccentricity*[stimulus.locations{stimulus.tmp.stimLocationindx(3)}],phaseNum);
 drawGabor(.00000001,stimulus.eccentricity*[stimulus.locations{stimulus.tmp.stimLocationindx(4)}],phaseNum);
...
if ~isfield(stimulus,'init') || 1
 ...
 stimulus.cueCondition = [1 2]; % attended (1,2) OR distributed (1,2)
...
if round(stimulus.nDisplayContrasts*desiredContrast/stimulus.currentMaxContrast)>stimulus.nDisplayContrasts
 disp(sprintf('(drawGabor) Desired contrast (%0.9f) out of range max contrast : %0.9f',desiredContrast,stimulus.currentMaxContrast));
 keyboard
end
 ...
 for hh = 1:indexes(1) % pedestal
  for jj = 1:indexes(2) % attentional condition
   if ~isempty(stimulus.quest.q{hh,jj})
    ...
    numberOfTrials(hh,jj) =  length(stimulus.quest.q{hh,jj}.intensity);
   end
  end
 end
 % making errorbars:
  ...
  for jj = 1:indexes(2) % attentional condition
   for ii = 1:indexes(3) % number of quest
    if ~isempty(stimulus.quest.q{hh,jj,ii})
     ...
     numberOfTrials(hh,jj,ii) =  length(stimulus.quest.q{hh,jj,ii}.intensity);
    end
   end
  end
 end
...
results.trialNum = numberOfTrials;

%  plot:
fv = smartfig('interimResults','reuse');
plotColor = {'r.', 'k.'};
...
xlabel('Stimulus contrast (%)','FontName','Arial','FontSize',Fsize);
...
set(gca,'FontName','Arial','FontSize',Fsize,'PlotBoxAspectRatio',[1 1 1], ...
 'LineWidth',thinLine,'TickLength',[0.025 .01], ...
 'XColor',XYColor,'XTick',[0.00022 stimulus.pedestals(2:end)],'XTickLabel',100*[0 stimulus.pedestals(2:end)], ...
 'YColor',XYColor,'Box','off', ...
 'YTick',[0.0002 0.002 0.02 0.2 0.4],'YTickLabel',100*[0.0002 0.002 0.02 0.2 0.4]);

% attended
ch = get(fv,'Child'); % getting lines, circles and squares
foh = findobj(ch,'Marker','o'); % getting circles
set(foh,'MarkerSize',markerSize,'MarkerFaceColor',myRED,'MarkerEdgeColor',myRED)

% neutral
foh = findobj(ch,'Marker','s'); % getting squares
