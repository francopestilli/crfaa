function results = runNestedHypothesisTest_r(varargin)
% function runStatTestAttention(varargin)
%
...
%  - /Users/frakkopesto/data/riken/crfaa/fmridata/crfaa/nestedHypothesisTest/figs_nestedFits_all;
%
% Results are saved in:
%  - /Users/frakkopesto/data/riken/crfaa/fmridata/crfaa/nestedHypothesisTest/allOBS_results;
% 
% It calls the fucntions:
% - makeFitSimIndividualPlotsNested.m
% - ...
%
% This code:
...
getArgs(varargin,{...
 ...
 'doTestOnly',0, ...
 'analysisType', 'ci',...'standard', ... % 12 or ci
 'crfFittype','nk2', ...
...
for o = 1:length(observer)      % do the whole thing for each observer
 disp(sprintf('[runNestedHypothesisTest_r] Running fit on Observer: %s',observer{o}))
 disp(sprintf('[runNestedHypothesisTest_r] Running fit on Observer: %s',observer{o}))
 disp(sprintf('[runNestedHypothesisTest_r] Running fit on Observer: %s',observer{o}))
 ...
 if strcmpi(observer{o},'avrg')
  switch analysisType
   case {'CorrectIncorrect' 'corrincorr' 'CorrIncorr' 'ci' '1st2ndInterval' '1st2nd' '12'}
    disp('[runNestedHypothesisTest_r] ERROR! only the 4 conditions analysis was computed for the average.')
    keyboard
  end
 end
 ...
 for t = 1:length(testType)
  ...
  %%%%%%%%%%%%%%%%%
  if saveData
   saveDataFolder = '/Users/frakkopesto/data/riken/crfaa/fmridata/crfaa/nestedHypothesisTest/';
