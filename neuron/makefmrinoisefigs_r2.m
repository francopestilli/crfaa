function makefmrinoisefigs_r2(varargin)
%
% function makefmrinoisefigs_r2({plotType},{observer},{visualarea},{adaptation})
%
...
getArgs(varargin,{...
 'observer',{'fm' 'fp' 'jg'}, ...
 'visualarea',{'v1'}, ...{'05' '06' '07' '075'}, OR  {'v1' 'v2' 'v3' 'v4'}
 'adaptation',{0},   ...
 'iscombined', 0,    ...
 'averageD',1,       ...
 'normalize',0,      ...
...
for v = 1:length(visualarea) % loop through each r2 roi
 ...
 % make attention plot %
 %%%%%%%%%%%%%%%%%%%%%%%
 figTag = sprintf('Att_r2%s_',visualarea{v});
 ...
 if doMeanObserver
  if length(adaptation) == 1
   keyboard
   % not sure about this
   %    meanNoise = squeeze(nanmean(attNorm_allSTD,2)); % mean across observers
   %    steNoise =  squeeze(nanstd(attNorm_allSTD,[],2))/sqrt(3); % ste acrss observers
  else  
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%
 % make adaptation plot:
 %%%%%%%%%%%%%%%%%%%%%%%%
 % average across observers and attention conditions.
 adpNorm_allSTD = zeros(size(allSTD));
 if normalize
  for o = 1:size(allSTD,2)  % observer
   for a = 1:size(allSTD,3) % attention
    adpNorm_allSTD(:,o,a) = allSTD(:,o,a)./allSTD(1,o,a); % normalizing by adapt-0
   end
  end
 else
  % NOT normalizing by the adapt-0 value
  adpNorm_allSTD = allSTD;
 end
 
 figTag = sprintf('Adp_r2%s_',visualarea{v});
 if doMeanObserver
  if length(adaptation) == 1
   keyboard
   % not sure about this
   %    meanNoise = squeeze(nanmean(adpNorm_allSTD,3)); % mean across attention
   %    steNoise =  squeeze(nanstd(adpNorm_allSTD,[],3))/sqrt(3); % ste acrss attention
  else
   meanNoise = squeeze(nanmedian(adpNorm_allSTD,2)); % mean across observers
   steNoiseAdp =  nanstd(meanNoise,[],2)/sqrt(3); % ste acrss observers
   meanNoiseAdp = mean(meanNoise,2);

  end
  noiseATTIndexPlot(meanNoiseAdp',steNoiseAdp',saveFig,figTag,averageD,normalize);
  ...
  if length(adaptation) == 1
   ...
   meanNoiseAdp = squeeze(nanmean(adpNorm_allSTD,3)); % mean across attention
   steNoiseAdp =  squeeze(nanstd(adpNorm_allSTD,[],3))./sqrt(3); % ste acrss observers
  end
  
  % plot
  for o = 1:size(allSTD,2)
   figTag1 = sprintf('%s%s%s',figTag,'_O',num2str(o));
   noiseATTIndexPlot(meanNoiseAdp(:,o)',steNoiseAdp(:,o)',saveFig,figTag1,averageD,normalize);
   figTag1 = [];
  end
 end
 ...
 if doOnePlot
  figurename = sprintf('avrgFmriNoise_%s','single');
  h = smartfig(figurename,'reuse');
  
  mybar([meanNoiseAtt;meanNoiseAdp'], ...
   'yError',[steNoiseAtt;steNoiseAdp'], ...
   'groupLabels',{'Attention','Adaptation'}, ...
   'withinGroupLabels',{'A/0%','D/28%','U/1005'}, ...
   'yAxisMin=0.5','yAxisMax=1.5');
