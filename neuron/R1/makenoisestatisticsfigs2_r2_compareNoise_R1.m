function makenoisestatisticsfigs2_r2_compareNoise_R1(varargin)
%
% function makenoisestatisticsfigs2_r2_compareNoise_R1({plotType},{observer},{visualarea},{adaptation})
%
...
%
% call type e.g.,
% makenoisestatisticsfigs2_r2_compareNoise_R1('observer',{'jg' 'fp' 'fm'},'visualarea',{'v1'},'adaptation',[0])
%
...
plotTvC=[];        % chose whether to plot the TvC for each test (A2A, D2D D2A)
...
if recomputeNoise
 ...
 for iObserver = 1:length(observer) % do it for each observer independently
  disp(sprintf('[makenoisestatisticsfigs2_r2_compareNoise_R1] OBS: %s',observer{iObserver}))
  for iArea = 1:length(visualarea)
   disp(sprintf('[makenoisestatisticsfigs2_r2_compareNoise_R1] AREA: %s',visualarea{iArea}))
   for iFit = 1:length(noiseTestType) % compute noise from one condition (D or A) and test it to the other
    ...
    disp(sprintf('[makenoisestatisticsfigs2_r2_compareNoise_R1] FIT: %s',noiseTestType{iFit}))
    
    % load the proper file.
    [d meanC events thisfullfile doMean] = loadData(observer{iObserver},visualarea{iArea}, ...
                                                    defaultDataFolder,adaptation,r2,subtractionType);
    ...
    disp(sprintf('[makenoisestatisticsfigs2_r2_compareNoise_R1] Loaded %s',thisfullfile))
    ...
    switch noiseTestType{iFit}
     case {'D2D'} % estimate noise from distributed and plot the crf
      disp(sprintf('[makenoisestatisticsfigs2_r2_compareNoise_R1] FITTING <Distributed Noise>'))
     ...
     case  {'D2A'} % noise was from distributed and it is now used to plot the crf
      disp(sprintf('[makenoisestatisticsfigs2_r2_compareNoise_R1] FITTING <Attended crf> using <Distributed Noise>'))
      % set up general structures and info
     ...
     case  {'A2A'} % estimate noise from attended and plot the crf
      disp(sprintf('[makenoisestatisticsfigs2_r2_compareNoise_R1] FITTING <Attended Noise>'))
    ...
    filename = [date,'CompareNoiseFit_Dnt_',dprimefittype,'_',r2,'_',subtractionType,'.mat'];
 ...
 keyboard
 % not worked on this yet after R1
...
% plot TvCs and CRFs
if plotTvC
 makeIndAttentionPlot(results,noise,observer,fitType,saveFig,plotLog,r2,subtractionType);
end

% make scatter plot
noiseScatterPlot(noise,saveFig,r2,subtractionType,dprimefittype)
...
%%%%%%%%%%%%%%%%%%%%%%%%
function makeIndAttentionPlot(results,noise,observer,fitType,saveFig,plotLog,r2,subtractionType,dprimefittype)
...
% noiseScatterPlot %
%%%%%%%%%%%%%%%%%%%%
function noiseScatterPlot(noise,saveFig,r2,subtractionType, dprimefittype)
...
% plot sigma
% attention figure
figurename = sprintf('attention_NoiseScatterPlot_sigma_r%s_%s_%s',r2,subtractionType,dprimefittype);
h = smartfig(figurename,'reuse');
set(h,'Name',figurename);

loglog([.007,.007;.18,.18],[.007,.007;.18,.18],'k-','Color',plotInfo.XYColor)
hold on;
...
% plot offset
figurename = sprintf('attention_NoiseScatterPlot_offset_r%s_%s_%s',r2,subtractionType,dprimefittype);
h = smartfig(figurename,'reuse');
set(h,'Name',figurename);
...
% loadData %
%%%%%%%%%%%%
function [d meanC events filename doMean] = loadData(observer,visualAreas,defaultDataFolder,adaptation,r2,subtractionType)
...
if strcmpi(observer,'avrg')
 % (0) load data file
 [filename vars2load] = makeFileName(observer,[],visualAreas,defaultDataFolder,r2,subtractionType);
 ...
 if strcmpi(visualAreas,'v1')
  [v2filename vars2load] = makeFileName(observer,[],'v2',defaultDataFolder,r2,subtractionType);
  [data] = load(sprintf('%s',v2filename),vars2load{1}, vars2load{2});
...
if (~strcmp(visualArea,'v1') && strcmp(r2,'070')) || (strcmp(visualArea,'v1') && strcmp(r2,'070'))
