function [noise exptData] = dprimefit3_r2_compareNoise_R1(data,varargin)
% dprimefit3_r2_compareNoise_R1.m
%
% R1 is the version modified at columbia for the NN rebuttal
%
%        $Id: dprimefit3_r2_compareNoise_R1.m
%      usage: dprimefit3_r2_compareNoise_R1(events,d1)
%         by: franco pestilli
%       date: 01/23/09
...
% check arguments
if nargin < 1
 help dprimefit3_r2_compareNoise_R1
 return
end
...
switch doNoise
 ...
 case {'tvc2crf'}
  ...
  if isempty(noise) % if noise is not empty it was passed and needs to be used to estimate the CRF
   ...
   doRecomputeR2 = 1;
   disp(sprintf('[dprimefit3_r2_compareNoise_R1] noise was not empty, but passed through, we are not going to fit...'))
  end
  ...
  if doRecomputeR2
   ...
   disp(sprintf('[dprimefit3_r2_compareNoise_R1] DONE computing the fit of the model given a passed noise: R2 = [%s]', ...
...
disp(sprintf('[dprimefit3_r2_compareNoise_R1:fitTVC] start, (%s)',dprimefittype));
% global figLabel;
...
switch dprimefittype
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise_R1:fitTVC] Unknown TvC psychoFittype: %s',dprimefittype));
  keyboard
end
...
if dispFit
 ...
 %disp(sprintf('[dprimefit3_r2_compareNoise_R1:fitTVC] showingthe fit: %s',dprimefittype));
...
disp(sprintf('[dprimefit3_r2_compareNoise_R1:fitTVC] done'));
...
if ~isfield(behavior,'tvcfit')
 %disp(sprintf('[dprimefit3_r2_compareNoise_R1] tvcfit has not been done'));
 keyboard
end
...
switch (behavior.tvcfit.type)
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise_R1:getThresholdContrast) Unknown TvC fittype: %s',behavior.tvcfit.type));
  keyboard
end
...
%disp(sprintf('[dprimefit3_r2_compareNoise_R1:makeModelCRFfromTvC] start'));
...
while (contrast(end) < maxC)
 ...
 %disp(sprintf('[dprimefit3_r2_compareNoise_R1:makeModelCRFfromTvC] current PedContrast %s and DeltaC %s',num2str(contrast(end)),num2str(deltaContrast)));
...
switch crffit.fittype
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise_R1:crf] Unknown crf fittype: %s',crffit.fittype));
  keyboard
end
...
switch (noise.noisetype)
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise_R1:crfsigma] Unknown model type %s',p.noisetype));
end
...
%disp(sprintf('[dprimefit3_r2_compareNoise_R1:computeTvC] start...'));
  ...
  for pedc = 1:length(exptData.pedestalsTvC)
   ...
   if doBootstrap
    ...
    for bootStrapN = 1:numBoostraps
     exitflag = 0;
     disp(sprintf('[dprimefit3_r2_compareNoise_R1] Pedestal <%s> - percent bootstrap done <%s>',num2str(whichPedc),num2str(100*bootStrapN/numBoostraps)))
    ...
    %disp(sprintf('[dprimefit3_r2_compareNoise_R1:computeTvC] no bootstrap, pedestal %0.4f',whichPedc))
    ...
    if isfield(exptData,'syntheticThresholdContrast')
     ...
     % disp(sprintf('[dprimefit3_r2_compareNoise_R1] Using synthetic threshold of %0.4f as initial threshold',exptData.syntheticThresholdContrast(pedc)));
    else
     % disp(sprintf('[dprimefit3_r2_compareNoise_R1] Using %0.4f as initial threshold',thresholdInit));
    end
    initParams = [thresholdInit 3.5 .05];
...
switch noisetype
 ...
 otherwise
  %disp(sprintf('(dprimefit3_r2_compareNoise_R1:getNoiseParams) Unknown model type %s',noisetype));
  keyboard
end
...
switch (noisetype)
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise_R1:computeNoiseValue] Unknown noisetype %s',noisetype));
  keyboard
end
...
disp('[dprimefit3_r2_compareNoise_R1:fitCRFfromTvC] fitting noise: CRF from TvC');
...
disp(sprintf('[dprimefit3_r2_compareNoise_R1:fitCRFfromTvC] DONE fitting noise: CRF from TvC: R2 = [%s]',num2str(noise.bestFit.r2)));
