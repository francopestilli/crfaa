function [d d1 ts events] = meanERA_ROI_remove0_script3_R1(groupNum,scanNum,r2thr,saveTag,target_roiFileNames)
% function [d d1 ts events] = meanERA_ROI_remove0_script3_R1(groupNum,scanNum,r2thr,saveTag,target_roiFileNames)
%
...
if ~(exist('mrSession.mat','file') == 2)
 disp(sprintf('[meanERA_ROI_remove0_script3_R1] mrSession.mat not found in this directory. please cd to a mrLoadret dir'));
 return
end
...
elseif nargin == 5
 disp(sprintf('[meanERA_ROI_remove0_script3_R1] all paramteters passed by users'));
 target_roiFileNames = switchVisualArea(target_roiFileNames);
 saveTag = ['2010-30-May','_',target_roiFileNames{1}(1:2),'_',saveTag];
 
else
 d  = [];d1  = [];ts  = [];events = [];
 disp(sprintf('[meanERA_ROI_remove0_script3_R1] ERROR ... nargin must be 3, 4 or 6 ...'));
 return
end
disp(sprintf('[meanERA_ROI_remove0_script3_R1] DO ANALYSIS ON group %i, scan %i roi: %s',groupNum,scanNum,target_roiFileNames{1}(1:2)));

% this is the original analysis
[d  ts  events] = DOerainroi0remove2_R1(target_roiFileNames,groupNum,scanNum, r2thr, sprintf('crfXloc4conditions_R1'));

% this is the analysis for correct and incorrect responses
% [d  ts  events] = DOerainroi0remove2_R1(target_roiFileNames,groupNum,scanNum, r2thr, sprintf('crfXloc4conditionsCorrIncor'));
...
% [d  ts  events] = DOerainroi0remove2_R1(target_roiFileNames,groupNum,scanNum, r2thr, sprintf('crfXloc4conditions1st2ndInterv'));
...
d1 = fitTimecourse(ts.subtracted_mean_roi_ts,ts.stimvol,tr,'concatInfo',ts.concatInfo,'displayFit=1', ...
    'fitType=glm','option=std','stdGroups',{1:8 9:16 17:24 25:32 });
...
disp(sprintf('[meanERA_ROI_remove0_script3_R1] Saving d, d1, ts and events as %s',saveTag));
eval(sprintf('save(''%s'',''d'', ''d1'',''ts'',''events'')', saveTag))

% % plot and save figures:roiTest
disp(sprintf('[meanERA_ROI_remove0_script3_R1] Plotting results of group %i, scan %i roi: %s',groupNum,scanNum,target_roiFileNames{1}(1:2)));
% % plot and save figures:roiTest
c = plotCRFtimecourse(d1,events,saveTag);

% plotting and saving figures:
disp(sprintf('[meanERA_ROI_remove0_script3_R1] DONE ANALYSIS ON group %i, scan %i roi: %s',groupNum,scanNum,target_roiFileNames{1}(1:2)));
disp(sprintf('[meanERA_ROI_remove0_script3_R1] pwd = [%s]',pwd));
