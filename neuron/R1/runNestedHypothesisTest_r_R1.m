function results = runNestedHypothesisTest_r_R1(varargin)
% function runNestedHypothesisTest_r_R1(varargin)
%
% version _r is the one used at riken.
% version R1 is the one used for the rebuttal at NN, May 2010
...
%   /data2/crfaa/crfaa/data_used_files_R1/
%
% Figures are saved in:
%  - /data2/crfaa/crfaa/nestedHypothesisTest_R1/figs_nestedFits_all;
%
% Results are saved in:
%  - /data2/crfaa/crfaa/nestedHypothesisTest_R1/allOBS_results;
% 
% It calls the fucntions:
% - makeFitSimIndividualPlotsNested_R1.m
% - ...
%
% This code:
% 1. it is written for the attention paper, and runs across
% attention only.
...
figDir = sprintf('figs_nestedAttTests_%s_%s_%s',r2,subtractionType,date);
...
for o = 1:length(observer)      % do the whole thing for each observer
 disp(sprintf('[runNestedHypothesisTest_r_R1] Running fit on Observer: %s',observer{o}))
 disp(sprintf('[runNestedHypothesisTest_r_R1] Running fit on Observer: %s',observer{o}))
 disp(sprintf('[runNestedHypothesisTest_r_R1] Running fit on Observer: %s',observer{o}))
 ...
 if strcmpi(observer{o},'avrg')
  switch analysisType
   case {'CorrectIncorrect' 'corrincorr' 'CorrIncorr' 'ci' '1st2ndInterval' '1st2nd' '12'}
    disp('[runNestedHypothesisTest_r_R1] ERROR! only the 4 conditions analysis was computed for the average.')
    keyboard
  end
 end
 ...
 for t = 1:length(testType)
  ...
  for v = 1:length(visualAreas) % v1-v4
   % load data:
   [d meanC] = loadData(observer{o},visualAreas{v},defaultDataFolder,adaptation,analysisType, r2, subtractionType);
   if isempty(d)
    return
   end
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   results{t}{o,v} =  makeFitSimIndividualPlotsNested_R1(d,meanC, ...
    'dispFit', 1, ...
   ...
   % makeFitSimIndividualPlotsNested_R1
   saveFitFig.testType   = fullTest;
   fullResults{t}{o,v} =  makeFitSimIndividualPlotsNested_R1(d,meanC, ...
    'dispFit', 0,...
    'crfFittype',crfFittype, ...
    'testType',fullTest, ...
    'visualArea',visualAreas{v}, ...
   ...
   figureName = [fileName,'_',analysisType,'_',r2,'_',visualAreas{v}];
   
   % save each figure:
   if ~doIndividualPlots
    if saveFig
     defaultDataFolder = '/data2/crfaa/crfaa/nestedHypothesisTest_R1/';
     figDir = sprintf('figs_nestedFits_all_%s_%s_%s',r2,subtractionType,date);
     savefig(results{end}{o,v}.plotInfo.figureHandle,'figName',[figureName,testType{t}],'defaultDataFolder',defaultDataFolder,'figDir',figDir);
    end
   end
  end
  ...
  %%%%%%%%%%%%%%%%%
  if saveData
   saveDataFolder = '/data2/crfaa/crfaa/nestedHypothesisTest_R1/';
   fiDir = sprintf('allOBS_results_%s_%s_%s',r2,subtractionType,date);
...
% loadData %
%%%%%%%%%%%%
function [d meanC] = loadData(observer,visualAreas,defaultDataFolder,adaptation,analysisType, r2, subtractionType)
...
if strcmpi(observer,'avrg')
 ...
 if strcmpi(visualAreas,'v1')
  [filename vars2load] = makeFileName(observer,[],'v2',defaultDataFolder,analysisType, '030', subtractionType);
  [data2] = load(sprintf('%s',filename),vars2load{1}, vars2load{2});
  meanC = data2.meanC{adp};
  clear data2
 else
...
if strcmpi(observer,'avrg')
 ...
 switch analysisType
  case {'standard' '4conditions' '4c' '4' 's'} % this is the original analysis for the main results
   if strcmp(visualArea, 'v1') && strcmp(r2,'070') % v1 at 0.7 r2 uses the original files
     filename = fullfile('/data2/crfaa/crfaa/data_used_files_R1/', ...
                sprintf('%s_A%i_2010-30-May_%s_era_R1_subtr%s_%s.mat',upper(observer), ...
                                   adaptation,visualArea,subtractionType,visualArea));

   elseif  ~strcmp(visualArea, 'v1') && strcmp(r2,'050') % v2-v3 at 0.5 uses original files
       filename = fullfile('/data2/crfaa/crfaa/data_used_files_R1/', ...
                sprintf('%s_A%i_2010-30-May_%s_era_R1_subtr%s_%s.mat',upper(observer), ...
                                   adaptation,visualArea,subtractionType,visualArea));
   else
