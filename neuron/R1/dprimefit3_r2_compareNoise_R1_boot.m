function [noise exptData] = dprimefit3_r2_compareNoise_R1_boot(data,varargin)
% dprimefit3_r2_compareNoise_R1_boot.m
%
% R1 is the version modified at columbia for the NN rebuttal
%
%        $Id: dprimefit3_r2_compareNoise_R1_boot.m
%      usage: dprimefit3_r2_compareNoise_R1_boot(events,d1)
%         by: franco pestilli
%       date: 01/23/09
...
iBoot=[];          % iBoot is the number of the current boot, this is used to re-load the TvC once computed for the first boot strap and speed up computer time
...
% check arguments
if nargin < 1
 help dprimefit3_r2_compareNoise_R1_boot
 return
end
...
switch doNoise
 ...
 case {'tvc2crf'}
  ...
  if isempty(noise) % if noise is not empty it was passed and needs to be used to estimate the CRF
   ...
   doRecomputeR2 = 1;
   disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot] noise was not empty, but passed through, we are not going to fit...'))
  end
  ...
  if doRecomputeR2
   ...
   disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot] DONE computing the fit of the model given a passed noise: R2 = [%s]', ...
...
disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:fitTVC] start, (%s)',dprimefittype));
% global figLabel;
...
switch dprimefittype
 case {'poly' 'polynonmial' 'pfit' 'p'}
  ...
  pedestals = pedestals(:);
  pedestals(1) = exptData.behavior.tvc.thisTvC(1)/2;
  
  % fit a polynomial:
  exptData.behavior.tvcfit.polyorder = 3;
 ...
 case {'skewedgaussian' 'sg' 'skew' 'gaussian' 'g'}
  ...
  % the skewed gaussian is not defined at 0
  % so i am using 1/2 the 0-ped deltac instead of zero to make 
  % the computations work. This 'lifts' up the 
  % lowest threshold of the estimated TvC (the nominal 0-ped)
  % but it makes the rest of the code work.
  TVC = exptData.behavior.tvc.thisTvC;%(2:end);
  TVC = TVC(:);
  pedestals = exptData.pedestalsTvC;%(2:end);
  pedestals = pedestals(:);
  ...
  exptData.behavior.tvcfit.fitParams.fit = fitParams;
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:fitTVC] Unknown TvC psychoFittype: %s',dprimefittype));
  keyboard
end
...
if dispFit
 ...
 %disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:fitTVC] showingthe fit: %s',dprimefittype));
...
disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:fitTVC] done'));
...
if ~isfield(behavior,'tvcfit')
 %disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot] tvcfit has not been done'));
 keyboard
end
...
switch (behavior.tvcfit.type)
 ...
 case {'skewedgaussian' 'sg' 'skew' 'gaussian' 'g'}
  threshold = skewFun(behavior.tvcfit.fitParams.fit.params,pedestal);
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:getThresholdContrast) Unknown TvC fittype: %s',behavior.tvcfit.type));
  keyboard
end
...
% disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:makeModelCRFfromTvC] start'));

% grab the dprime
dprime = exptData.behavior.dprime;

% grab the tvc info 
behavior = exptData.behavior;
behavior.pedestals = exptData.pedestals;

% we always start the curve at a response of 0
contrast = exptData.pedestalsCRF(1);
if contrast == 0
 contrast =  exptData.behavior.tvc.thisTvC(1)/2;
end

% special case to handle JG and FM for the poly fit:
switch exptData.behavior.tvcfit.type
 case {'poly' 'polynonmial' 'pfit' 'p'}
  maxC = .89;
 otherwise
  maxC = 1;
...
deltaContrast = 1;
while (contrast(end) < maxC)
 % get the threshold contrast for this pedestal from the fit
 % of the behaviorally measured TvC function
 deltaContrast = getThresholdContrast(contrast(end),behavior);
 
 %  disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:makeModelCRFfromTvC] current DeltaC %s',num2str(deltaContrast)));
...
if any(isnan(contrast)) || any(isnan(response))
 keyboard
end
...
filename = '/data2/crfaa/crfaa/data_used_files/12-Jul-2009averageTvC.mat';
...
switch crffit.fittype
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:crf] Unknown crf fittype: %s',crffit.fittype));
  keyboard
end
...
switch (noise.noisetype)
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:crfsigma] Unknown model type %s',p.noisetype));
end
...
%disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:computeTvC] start...'));
  ...
  for pedc = 1:length(exptData.pedestalsTvC)
   ...
   if doBootstrap
    ...
    for bootStrapN = 1:numBoostraps
     exitflag = 0;
     disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot] Pedestal <%s> - percent bootstrap done <%s>',num2str(whichPedc),num2str(100*bootStrapN/numBoostraps)))
    ...
    %disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:computeTvC] no bootstrap, pedestal %0.4f',whichPedc))
    ...
    if isfield(exptData,'syntheticThresholdContrast')
     ...
     % disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot] Using synthetic threshold of %0.4f as initial threshold',exptData.syntheticThresholdContrast(pedc)));
    else
     % disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot] Using %0.4f as initial threshold',thresholdInit));
    end
    initParams = [thresholdInit 3.5 .05];
...
switch noisetype
 ...
 otherwise
  %disp(sprintf('(dprimefit3_r2_compareNoise_R1_boot:getNoiseParams) Unknown model type %s',noisetype));
  keyboard
end
...
switch (noisetype)
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:computeNoiseValue] Unknown noisetype %s',noisetype));
  keyboard
end
...
disp('[dprimefit3_r2_compareNoise_R1_boot:fitCRFfromTvC] fitting noise: CRF from TvC');
...
disp(sprintf('[dprimefit3_r2_compareNoise_R1_boot:fitCRFfromTvC] DONE fitting noise: CRF from TvC: R2 = [%s]',num2str(noise.bestFit.r2)));
...
if any(isnan(c)) || any(isnan(r)) || any(isnan(pedC))
 keyboard
else
...
if any(isnan(modelResponse)) || any(isnan(residual))
 keyboard
end
