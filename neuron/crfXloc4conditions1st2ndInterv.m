function d = crfXloc4conditions1st2ndInterv(d,location)
% function d = crfXloc4conditions1st2ndInterv(d,location)
%
...
% 2. collect CRF for:
%    cue=1 location=target    (Attended 1st interval)
%    cue=1 location=target    (Attended 2nd interval)
%    cue=1 location=notTarget (Unattended)
%    cue=2 location=target    (Distributed Target 1st interval)
%    cue=2 location=target    (Distributed Target 2nd interval)
...
disp(sprintf('[crfXloc4conditions1st2ndInterv: LOC%s] Pedestal contrasts: %s',num2str(location),num2str(pedestals)));

% set the number of HRF (length(pedestals) X 7 attentional conditions,; [At1st, At2nd, U, Dt1st, Dt2nd, Dnt])
...
for runNum = 1:length(d.stimfile)
 ...
 for tNum = 1:e{runNum}(phaseNum).nTrials
  ...
  % plase nott he the following indexes are changed during the contrast
  % response function fitting procedure
  
  %%% This volume is attended
  if (cueCondition == 1) && (targetLocation == location)
   % the deltac was in the first interval on this trial
   if targetInterval == 1
    % (1) Focal cue, target 1st interval
    pedNum  = e{runNum}(phaseNum).parameter.PedContrast(tNum);
    conditionNum = pedNum;
    thisContrast = e{runNum}(phaseNum).traces(location,tNum)-deltaC;
    if dispON, disp(sprintf('[crfXloc4conditions1st2ndInterv: LOC%s] Attended    1st - Scan: %s Trial: %s Contrast: %0.4f deltaC: %0.4f PedNum: %s hdrNum: %s', ...
      num2str(location),num2str(runNum),num2str(tNum),thisContrast,deltaC,num2str(pedNum),num2str(conditionNum))); end
    ...
    thisContrast = e{runNum}(phaseNum).traces(location,tNum)-deltaC;
    if dispON, disp(sprintf('[crfXloc4conditions1st2ndInterv: LOC%s] Attended    2nd - Scan: %s Trial: %s Contrast: %0.4f deltaC: %0.4f PedNum: %s hdrNum: %s',...
      num2str(location),num2str(runNum),num2str(tNum),thisContrast,deltaC,num2str(pedNum),num2str(conditionNum))); end
   end
  ...
  elseif (cueCondition == 1) && (targetLocation ~= location)
   ...
   if dispON, disp(sprintf('[crfXloc4conditions1st2ndInterv: LOC%s] Unattended     - Scan: %s Trial: %s Contrast: %0.4f deltaC: %0.4f PedNum: %s hdrNum: %s', ...
     num2str(location),num2str(runNum),num2str(tNum),thisContrast,deltaC,num2str(pedNum),num2str(conditionNum))); end
  ...
  elseif (cueCondition == 2) && (targetLocation == location)
   ...
   if targetInterval == 1
    ...
    if dispON, disp(sprintf('[crfXloc4conditions1st2ndInterv: LOC%s] Distributed  T-1st - Scan: %s Trial: %s Contrast: %0.4f deltaC: %0.4f PedNum: %s hdrNum: %s', ...
      num2str(location),num2str(runNum),num2str(tNum),thisContrast,deltaC,num2str(pedNum),num2str(conditionNum))); end
    ...
    if dispON, disp(sprintf('[crfXloc4conditions1st2ndInterv: LOC%s] Distributed  T-2nd - Scan: %s Trial: %s Contrast: %0.4f deltaC: %0.4f PedNum: %s hdrNum: %s', ...
      num2str(location),num2str(runNum),num2str(tNum),thisContrast,deltaC,num2str(pedNum),num2str(conditionNum))); end
   end
  ...
  elseif (cueCondition == 2) && (targetLocation ~= location)
   ...
   if dispON, disp(sprintf('[crfXloc4conditions1st2ndInterv: LOC%s] Distributed NT - Scan: %s Trial: %s Contrast: %0.4f deltaC: %0.4f PedNum: %s hdrNum: %s', ...
     num2str(location),num2str(runNum),num2str(tNum),thisContrast,deltaC,num2str(pedNum),num2str(conditionNum))); end
  else
   
   thisContrast = e{runNum}(phaseNum).traces(location,tNum);
   if dispON, disp(sprintf('[crfXloc4conditions1st2ndInterv: LOC%s] ERROR NO MATCH FOR THIS VOLUME! -Scan: %s Trial: %s Contrast: %0.4f deltaC: %0.4f PedNum: %s hdrNum: %s', ...
     num2str(location),num2str(runNum),num2str(tNum),thisContrast,deltaC,num2str(pedNum),num2str(conditionNum))); end
   keyboard
  end
 ...
 disp(sprintf('[crfXloc4conditions1st2ndInterv: LOC%i] volumes sorted out for SCAN%i',location,runNum));
