function makenoisestatisticsfigs2_r2_decision(varargin)
%
% function makenoisestatisticsfigs2_r2_decision({plotType},{observer},{visualarea},{adaptation})
%
% this function is an evolution of makenoisestatisticsfigs2_r2_compareNoise.m
% it does the noise comparison and plots using the decision model (noise is not early but late)
% this is after david's comments.
%
%
% this function calls dprimefit3_r2_decision.m
...
%
% call type e.g.,
% makenoisestatisticsfigs2_r2_decision('observer',{'jg' 'fp' 'fm'},'visualarea',{'v1'},'adaptation',[0])
%
...
if recomputeNoise
 ...
 for iObserver = 1:length(observer) % do it for each observer independently
  disp(sprintf('[makenoisestatisticsfigs2_r2_decision] OBS: %s',observer{iObserver}))
  for iArea = 1:length(visualarea)
   disp(sprintf('[makenoisestatisticsfigs2_r2_decision] AREA: %s',visualarea{iArea}))
...
%     noise{iFit}.responseOffset = nan.*ones(length(observer),length(visualarea));
    disp(sprintf('[makenoisestatisticsfigs2_r2_decision] FIT: %s',noiseTestType{iFit}))
    ...
    disp(sprintf('[makenoisestatisticsfigs2_r2_decision] Loaded %s',thisfullfile))
    ...
    switch noiseTestType{iFit}
     case {'D2D'} % estimate noise from distributed and plot the crf
      disp(sprintf('[makenoisestatisticsfigs2_r2_decision] FITTING <Distributed Noise>'))
      
      % set up general structures and info
      fig = setUpPlot(observer{iObserver},whichCRF{1},adaptation,savePlots, dispFit);
      % NB this functions should call dprimefit3_r2_decision.m 
      % for the moment we are running the model fowrard so it is calling
      % <lateNoise>
      [n exptData] = lateNoise(data{iFit, iObserver, iArea}, ...
       'doNoise',fitType{1},          ...
     ...
     case  {'D2A'} % noise was from distributed and it is now used to plot the crf
      disp(sprintf('[makenoisestatisticsfigs2_r2_decision] FITTING <Attended crf> using <Distributed Noise>'))
      % set up general structures and info
      fig = setUpPlot(observer{iObserver},whichCRF{2},adaptation,savePlots, dispFit);
      
      [n exptData] = dprimefit3_r2_decision(data{iFit, iObserver, iArea}, ...
       'doNoise',fitType{1},          ...
     ...
     case  {'A2A'} % estimate noise from attended and plot the crf
      disp(sprintf('[makenoisestatisticsfigs2_r2_decision] FITTING <Attended Noise>'))
