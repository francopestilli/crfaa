function [noise exptData] = dprimefit3_r2_compareNoise(data,varargin)
% dprimefit3_r2_compareNoise.m
%
%        $Id: dprimefit3_r2_compareNoise.m
%      usage: dprimefit3_r2_compareNoise(events,d1)
%         by: franco pestilli
%       date: 01/23/09
...
% check arguments
if nargin < 1
 help dprimefit3_r2_compareNoise
 return
end
...
switch doNoise
 ...
 case {'tvc2crf'}
  ...
  if isempty(noise) % if noise is not empty it was passed and needs to be used to estimate the CRF
   ...
   doRecomputeR2 = 1;
   disp(sprintf('[dprimefit3_r2_compareNoise] noise was not empty, but passed through, we are not going to fit...'))
  end
  ...
  if doRecomputeR2
   ...
   disp(sprintf('[dprimefit3_r2_compareNoise] DONE computing the fit of the model given a passed noise: R2 = [%s]', ...
...
% disp(sprintf('[dprimefit3_r2_compareNoise:fitTVC] start, (%s)',dprimefittype));
% global figLabel;
...
switch dprimefittype
 case {'poly' 'polynonmial' 'pfit' 'p'}
  ...
  fitParams = fitpolynomial(pedestals,TVC,exptData.behavior.tvcfit.polyorder);
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise:fitTVC] Unknown TvC psychoFittype: %s',dprimefittype));
  keyboard
end
...
if dispFit
 ...
 %disp(sprintf('[dprimefit3_r2_compareNoise:fitTVC] showingthe fit: %s',dprimefittype));
...
disp(sprintf('[dprimefit3_r2_compareNoise:fitTVC] done'));
...
if ~isfield(behavior,'tvcfit')
 %disp(sprintf('[dprimefit3_r2_compareNoise] tvcfit has not been done'));
 keyboard
end
...
switch (behavior.tvcfit.type)
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise:getThresholdContrast) Unknown TvC fittype: %s',behavior.tvcfit.type));
  keyboard
end
...
% disp(sprintf('[dprimefit3_r2_compareNoise:makeModelCRFfromTvC] start'));
...
while (contrast(end) < maxC)
 ...
 %  disp(sprintf('[dprimefit3_r2_compareNoise:makeModelCRFfromTvC] current DeltaC %s',num2str(deltaContrast)));
...
if strcmp(strtrim(computer),'ticonzero.local')
 ...
 filename = '/Volumes/homosacer_data/riken/crfaa/fmridata/crfaa/data_used_files/12-Jul-2009averageTvC.mat';

elseif isfile('/Volumes/data/riken/crfaa/fmridata/crfaa/data_used_files/12-Jul-2009averageTvC.mat') % assuming this is run on homoacer at riken
 filename = '/Volumes/data/riken/crfaa/fmridata/crfaa/data_used_files/12-Jul-2009averageTvC.mat';
 
elseif isfile('/data2/crfaa/crfaa/data_used_files/12-Jul-2009averageTvC.mat')
...
switch crffit.fittype
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise:crf] Unknown crf fittype: %s',crffit.fittype));
  keyboard
end
...
switch (noise.noisetype)
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise:crfsigma] Unknown model type %s',p.noisetype));
end
...
%disp(sprintf('[dprimefit3_r2_compareNoise:computeTvC] start...'));
  ...
  for pedc = 1:length(exptData.pedestalsTvC)
   ...
   if doBootstrap
    ...
    for bootStrapN = 1:numBoostraps
     exitflag = 0;
     disp(sprintf('[dprimefit3_r2_compareNoise] Pedestal <%s> - percent bootstrap done <%s>',num2str(whichPedc),num2str(100*bootStrapN/numBoostraps)))
    ...
    %disp(sprintf('[dprimefit3_r2_compareNoise:computeTvC] no bootstrap, pedestal %0.4f',whichPedc))
    ...
    if isfield(exptData,'syntheticThresholdContrast')
     ...
     % disp(sprintf('[dprimefit3_r2_compareNoise] Using synthetic threshold of %0.4f as initial threshold',exptData.syntheticThresholdContrast(pedc)));
    else
     % disp(sprintf('[dprimefit3_r2_compareNoise] Using %0.4f as initial threshold',thresholdInit));
    end
    initParams = [thresholdInit 3.5 .05];
...
switch noisetype
 ...
 otherwise
  %disp(sprintf('(dprimefit3_r2_compareNoise:getNoiseParams) Unknown model type %s',noisetype));
  keyboard
end
...
switch (noisetype)
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_compareNoise:computeNoiseValue] Unknown noisetype %s',noisetype));
  keyboard
end
...
disp('[dprimefit3_r2_compareNoise:fitCRFfromTvC] fitting noise: CRF from TvC');
...
disp(sprintf('[dprimefit3_r2_compareNoise:fitCRFfromTvC] DONE fitting noise: CRF from TvC: R2 = [%s]',num2str(noise.bestFit.r2)));
