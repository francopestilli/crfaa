function r = makeFitSimIndividualPlotsNested(data,meanC,varargin)
% function r = makeFitSimIndividualPlotsNested(data,meanC,varargin)
%
...
% check arguments
if nargin < 2
 help makeFitSimIndividualPlotsNested
 return
end
...
defaultDataFolder = '/data2/crfaa/crfaa/nestedHypothesisTest/';
figDir = ['figs_nestedFit_eachFit_',analysisType]; % figures with individual function fits are saved here
figDirEach = ['figs_nestedFit_individual_',analysisType]; % fits are saved here
...
disp(sprintf('[makeFitSimIndividualPlotsNested:fitCRF] Using function: <%s>,  with test: <%s> start...',crfFittype,testType,saveFitFig.testType));
...
switch crfFittype
 ...
 otherwise
  disp(sprintf('[makeFitSimIndividualPlotsNested:fitCRF] %s fit type not defined',crfFittype))
  keyboard
end

disp(sprintf('[makeFitSimIndividualPlotsNested:fitCRF] Using function: <%s>,  with test: <%s> done...',crfFittype,saveFitFig.testType));
...
if ~(length(initparams) == wantedParams)
 disp(sprintf('[makeFitSimIndividualPlotsNested] NOT the right number of parameters - TestType: %s, Number of wanted parameters: %s, Number of parameters: %s', ...
