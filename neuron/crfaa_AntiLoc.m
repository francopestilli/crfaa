...
%
function myscreen = crfaa_AntiLoc(observer)
...
datadirname = fullfile(thisdir,'data',[observer,'AntiLocalizer']);
...
task{1}{1}.seglen   =   repmat(stimulus.dutyCycle,1,stimulus.period);
[task{1}{1} myscreen] = initTask(task{1}{1},myscreen,@localizerStartSegmentCallback,@localizerDrawStimulusCallback);

%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% run the eye calibration
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
myscreen.background = stimulus.grayColor;
...
while (phaseNum <= length(task{1})) && ~myscreen.userHitEsc
    % update the STIMULI PRESENTATION
...
setGammaTable(stimulus.localizerContrast);
mglFixationCross(1,1,stimulus.black);

stimulus.tmp.stimON_OFF = isodd(task.thistrial.thisseg); % odd is stimulus on, even is stimulus off
stimulus.tmp.stim_antistim = (task.thistrial.thisseg <= (stimulus.period));

% if stimulus.tmp.stimON_OFF, disp(sprintf('Stim is NOW on, (%i)',stimulus.tmp.stimON_OFF)); else disp(sprintf('Stim is NOW on, (%i)',stimulus.tmp.stimON_OFF)); end
% if stimulus.tmp.stim_antistim, disp(sprintf('STIMULUS, (%i)',stimulus.tmp.stim_antistim)); else disp(sprintf('ANTISTIMULUS, (%i)',stimulus.tmp.stim_antistim)); end
...
if stimulus.tmp.stimON_OFF
    if stimulus.tmp.stim_antistim
        % STIMULI ON
        phaseNum = mod(floor(2*mglGetSecs(task.thistrial.trialstart)/stimulus.flickerPeriod),2)+1;
        drawGabor(stimulus.localizerContrast,stimulus.eccentricity*[stimulus.locations{1}],phaseNum);
        drawGabor(stimulus.localizerContrast,stimulus.eccentricity*[stimulus.locations{2}],phaseNum);
        drawGabor(stimulus.localizerContrast,stimulus.eccentricity*[stimulus.locations{3}],phaseNum);
        drawGabor(stimulus.localizerContrast,stimulus.eccentricity*[stimulus.locations{4}],phaseNum);
        mglFillOval(0, 0, stimulus.fixOvalsz,stimulus.grayColor);
%         mglFixationCross(1,1,stimulus.black);

    else
        % ANTISTIMULI ON
        phaseNum = mod(floor(2*mglGetSecs(task.thistrial.trialstart)/stimulus.flickerPeriod),2)+1;
        drawAntiGrating(stimulus.localizerContrast,[0 0],phaseNum);
        mglBltTexture(stimulus.gausstex,stimulus.eccentricity*[stimulus.locations{1}]);
        mglBltTexture(stimulus.gausstex,stimulus.eccentricity*[stimulus.locations{2}]);
        mglBltTexture(stimulus.gausstex,stimulus.eccentricity*[stimulus.locations{3}]);
        mglBltTexture(stimulus.gausstex,stimulus.eccentricity*[stimulus.locations{4}]);
        mglFillOval(0, 0, stimulus.fixOvalsz,stimulus.grayColor);
%         mglFixationCross(1,1,stimulus.black);
    end
...
disppercent(-inf,'Calculating stimuli...');

% calculate a grating and gaussian envelope (gaussian is in the alpha channel)
g = (makeGaussian(stimulus.width,stimulus.height,stimulus.gaussSdx,stimulus.gaussSdy) > exp(-14/2));
theGaussian(:,:,1) = 128*ones(size(g));
theGaussian(:,:,2) = theGaussian(:,:,1);
theGaussian(:,:,3) = theGaussian(:,:,1);
theGaussian(:,:,4) = 255*g;
...
for thisPhase = 1:length(stimulus.phase)
    ...
    grating{thisPhase}(:,:,4) = 255*g;

    % anti-stimuli
    antigratingMatrix{thisPhase} = makeGrating(stimulus.antiwidth,stimulus.antiheight,stimulus.sf,stimulus.orientation,stimulus.phase(thisPhase));
end
clear g;

% making the texture for all the stimuli:
for thisPhase = 1:length(stimulus.phase)
    for thisContrast = [1 stimulus.deltaGratingColors]
        %% stimulus.texture
        ...
        stimulus.stimtex{thisPhase}(thisContrast+1) = mglCreateTexture(grating{thisPhase});

        antigrating{thisPhase}(:,:,1) = stimulus.midGratingColors+antigratingMatrix{thisPhase}*thisContrast;
        antigrating{thisPhase}(:,:,2) = antigrating{thisPhase}(:,:,1);
        antigrating{thisPhase}(:,:,3) = antigrating{thisPhase}(:,:,1);
        stimulus.antitex{thisPhase}(thisContrast+1) = mglCreateTexture(antigrating{thisPhase});

        disppercent(thisContrast/stimulus.deltaGratingColors);
    end
end

stimulus.gausstex = mglCreateTexture(theGaussian);
...
mglBltTexture(stimulus.stimtex{phase}(displayContrastNum+1),position);

%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function to draw the gabor stimulus
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function drawAntiGrating(desiredContrast,position,phase)
% drawAntiGrating
%
%        $Id: drawAntiGrating.m, v 1 2007/01/18 19:40:56 ?? ?? $
%      usage: drawAntiGrating(desiredContrast,position,pedestalNum,phase)
%         by: justin gardner, modified by franco pestilli
%       date: 01/11/07 - 01/18/07
%  copyright: (c) 2007 Justin Gardner
%    purpose: draw a grating on the screen with a specified contrast
%             within a given clut range (it finds the closest contrast to
%             the requested one within the available clut range)
...
mglBltTexture(stimulus.antitex{phase}(displayContrastNum+1),position);
