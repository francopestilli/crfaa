% dprimefit3_r2_decision.m
%
%        $Id: dprimefit3_r2_decision.m
%      usage: dprimefit3_r2_decision(events,d1)
%         by: franco pestilli
%       date: 01/23/09
...
function [noise exptData] = dprimefit3_r2_decision(data,varargin)
...
switch doNoise
 ...
 case {'tvc2crf'}
  ...
  if isempty(noise) % if noise is not empty it was passed and needs to be used to estimate the CRF
   ...
   doRecomputeR2 = 1;
   disp(sprintf('[dprimefit3_r2_decision] noise was not empty, but passed through, we are not going to fit...'))
  end
  ...
  if doRecomputeR2
   ...
   disp(sprintf('[dprimefit3_r2_decision] DONE computing the fit of the model given a passed noise: R2 = [%s]', ...
...
% disp(sprintf('[dprimefit3_r2_decision:fitTVC] start, (%s)',dprimefittype));
% global figLabel;
...
switch dprimefittype
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_decision:fitTVC] Unknown TvC psychoFittype: %s',dprimefittype));
  keyboard
end
...
if dispFit
 ...
 %disp(sprintf('[dprimefit3_r2_decision:fitTVC] showingthe fit: %s',dprimefittype));
...
disp(sprintf('[dprimefit3_r2_decision:fitTVC] done'));
...
if ~isfield(behavior,'tvcfit')
 %disp(sprintf('[dprimefit3_r2_decision] tvcfit has not been done'));
 keyboard
end
...
switch (behavior.tvcfit.type)
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_decision:getThresholdContrast) Unknown TvC fittype: %s',behavior.tvcfit.type));
  keyboard
end
...
% disp(sprintf('[dprimefit3_r2_decision:makeModelCRFfromTvC] start'));
...
while (contrast(end) < 1) %
 ...
 %  disp(sprintf('[dprimefit3_r2_decision:makeModelCRFfromTvC] current DeltaC %s',num2str(deltaContrast)));
...
switch crffit.fittype
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_decision:crf] Unknown crf fittype: %s',crffit.fittype));
  keyboard
end
...
switch (noise.noisetype)
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_decision:crfsigma] Unknown model type %s',p.noisetype));
end
...
%disp(sprintf('[dprimefit3_r2_decision:computeTvC] start...'));
  ...
  for pedc = 1:length(exptData.pedestalsTvC)
   ...
   if doBootstrap
    ...
    for bootStrapN = 1:numBoostraps
     exitflag = 0;
     disp(sprintf('[dprimefit3_r2_decision] Pedestal <%s> - percent bootstrap done <%s>',num2str(whichPedc),num2str(100*bootStrapN/numBoostraps)))
    ...
    %disp(sprintf('[dprimefit3_r2_decision:computeTvC] no bootstrap, pedestal %0.4f',whichPedc))
    ...
    if isfield(exptData,'syntheticThresholdContrast')
     ...
     % disp(sprintf('[dprimefit3_r2_decision] Using synthetic threshold of %0.4f as initial threshold',exptData.syntheticThresholdContrast(pedc)));
    else
     % disp(sprintf('[dprimefit3_r2_decision] Using %0.4f as initial threshold',thresholdInit));
    end
    initParams = [thresholdInit 3.5 .05];
...
switch noisetype
 ...
 otherwise
  %disp(sprintf('(dprimefit3_r2_decision:getNoiseParams) Unknown model type %s',noisetype));
  keyboard
end
...
switch (noisetype)
 ...
 otherwise
  %disp(sprintf('[dprimefit3_r2_decision:computeNoiseValue] Unknown noisetype %s',noisetype));
  keyboard
end
...
disp('[dprimefit3_r2_decision:fitCRFfromTvC] fitting noise: CRF from TvC');
...
disp(sprintf('[dprimefit3_r2_decision:fitCRFfromTvC] DONE fitting noise: CRF from TvC: R2 = [%s]',num2str(noise.bestFit.r2)));
