function results = adp_runAdaptationNestedHypothesisTest_r(varargin)
% function adp_runAdaptationNestedHypothesisTest_r(varargin)
%
...
% parameters. 
%
% THIS VERSION WORKS ACROSS ADAPTATION, 
% it runs a test of contrast gain vs. response or baseline shift
%
...
%   /data1/crfaa/crfaa/data_used_files/
%
% Figures are saved in:
%  - /data1/crfaa/crfaa/adp_nestedHypothesisTest/figs_nestedFits_all;
%
% Results are saved in:
%  - /data1/crfaa/crfaa/adp_nestedHypothesisTest/allOBS_results;
% 
...
r2                = []; % r2 value of te roi to load
subtractionType   = []; % ORIG or BYTRIAL, this is the way the times-series were 0-subtracted check runAllAnalyses_r_R1.m
testAdaptation    = []; % if 1 test across adaptation conditions, if 0 tests for the attentional effect in the passed adaptation condition
...
for o = 1:length(observer)      % do the whole thing for each observer
 disp(sprintf('[adp_runAdaptationNestedHypothesisTest_r] Running fit on Observer: %s',observer{o}))
 disp(sprintf('[adp_runAdaptationNestedHypothesisTest_r] Running fit on Observer: %s',observer{o}))
 disp(sprintf('[adp_runAdaptationNestedHypothesisTest_r] Running fit on Observer: %s',observer{o}))
 ...
 if strcmpi(observer{o},'avrg')
  switch analysisType
   case {'CorrectIncorrect' 'corrincorr' 'CorrIncorr' 'ci' '1st2ndInterval' '1st2nd' '12'}
    disp('[adp_runAdaptationNestedHypothesisTest_r] ERROR! only the 4 conditions analysis was computed for the average.')
    keyboard
  end
 end
 ...
 for t = 1:length(testType)
  ...
  for v = 1:length(visualAreas) % v1-v4
   ...
   % load data:
   defaultDataFolder = adp_defaultDataFolder;
   [d meanC] = loadData(observer{o},visualAreas{v},defaultDataFolder,adaptation,analysisType, r2, subtractionType, testAdaptation);
   if isempty(d) || isempty(meanC)
    disp(sprintf('[adp_runAdaptationNestedHypothesisTest_r] WARNING! Subject fm does not have the 100%% adaptation, exiting.\n[adp_runAdaptationNestedHypothesisTest_r] To incur in no problems please run fit on observer fm as last in a series of observers.'));
    return
   end
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   if testAdaptation
    % this function tests across adaptation conditions
    results{t}{o,v} =  adp_makeFitSimIndividualPlotsNested(d,meanC, ...
     'dispFit',        1,              ...
     ...
     'testAdaptation', testAdaptation);
   else
    % this function tests across attention conditions
   ...
   if testAdaptation
    % this function tests across adaptation conditions
    fullResults{t}{o,v} =  adp_makeFitSimIndividualPlotsNested(d,meanC, ...
     'dispFit',        0,              ...
     'crfFittype',     crfFittype,     ...
     'testType',       fullTest,       ...
     'visualArea',     visualAreas{v}, ...
   ...
   if testAdaptation
    fileName = sprintf('%s_%s_adp_nestedFTest',observer{o},deblank(thisDate));
   else
   ...
   if ~doIndividualPlots
    if saveFig
     defaultDataFolder = fullfile(adp_defaultDataFolder,'adp_nestedHypothesisTest/');
  ...
  %%%%%%%%%%%%%%%%%
  if saveData
   saveDataFolder = fullfile(adp_defaultDataFolder,'adp_nestedHypothesisTest/');
...
% loadData %
%%%%%%%%%%%%
function [d meanC] = loadData(observer,visualAreas,defaultDataFolder,adaptation,analysisType, r2, subtractionType, testAdaptation)
...
if strcmpi(observer,'avrg')
 % (0) load data file
 [filename vars2load] = makeFileName(observer,[],visualAreas,defaultDataFolder, analysisType, r2, subtractionType);
 
 if ~isfile(filename)
  disp(sprintf('[%s] Could not find file %s, exiting.',mfilename,filename));
  d = [];meanC = [];
  keyboard
 end
 [data] = load(sprintf('%s',filename),vars2load{1}, vars2load{2});
 
 % if we are testing across adaptation conditions
 % the return the whole d structure so that we can set up a test across
 ...
 if strcmpi(visualAreas,'v1')
  [filename vars2load] = makeFileName(observer,[],'v2',defaultDataFolder,analysisType, 'none', subtractionType);
  [data] = load(sprintf('%s',filename),vars2load{1}, vars2load{2});
  if testAdaptation
   meanC = data.meanC;
  else
 ...
 if testAdaptation
  % not implemented on individual observers because I would need to load
  % several files at once
  keyboard
 end
 
 if ~(strcmpi('fm',observer) && adaptation == 100)
  [filename vars2load] = makeFileName(observer,adaptation,visualAreas,defaultDataFolder,analysisType, r2, subtractionType);
  data = load(sprintf('%s',filename),vars2load{1}, vars2load{2});
...
function [filename vars2load] = makeFileName(observer,adaptation,visualArea,defaultDataFolder, ...
                                                      analysisType,r2, subtractionType)
 ...
 switch analysisType
  case {'standard' '4conditions' '4c' '4' 's'} % this is the original analysis for the main results
   if strcmp(r2,'none')
    ...
    filename = fullfile(adp_defaultDataFolder, ...
    ...
    filename = fullfile(adp_defaultDataFolder, ...
     sprintf('%s_A%i_2010-30-May_%s_era_R1_subtr%s_%sr%s.mat',upper(observer), ...
     adaptation,visualArea,subtractionType,visualArea,r2));
   end
