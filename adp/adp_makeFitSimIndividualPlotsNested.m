function r = adp_makeFitSimIndividualPlotsNested(data,meanC,varargin)
% function r = adp_makeFitSimIndividualPlotsNested(data,meanC,varargin)
%
% this function is called by adp_runNestedHypothesisTest_r.m.
%
...
% check arguments
if nargin < 2
 help adp_makeFitSimIndividualPlotsNested
 return
end
...
testAdaptation = []; % do a test across adaptation or attention
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
defaultDataFolder = '/dev/data/crfaa/adaptation_ms/data/hrfs_behavior/';
figDir     = ['adp_figs_nestedFit_eachFit_',analysisType]; % figures with individual function fits are saved here
figDirEach = ['adp_figs_nestedFit_individual_',analysisType]; % fits are saved here
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
d = setDataStructure(data, meanC, visualArea,analysisType, testAdaptation);
...
%%%%%%%%%%%%%%%%%%%
if dispFit
 p = dispCRF(r.d,r.crf,testType,saveFig,observer,visualArea);
end
...
%        dispCRF         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
function r = dispCRF(r,crf,testType,saveFig,observer,visualArea)
global defaultDataFolder figDirEach
adaptation = {'0' '28' '100'};
i = 1;

% bring up a new figure for each adaptation condition and do the plot on it:
figName = sprintf('%scrf_simFit_%sAdapt%s%s', ...
 upper(observer),testType,adaptation{i},visualArea);
...
% % % % % % %  figDirEach = 'figs_nestedFit_individual';
 figDir = savefig(h,'figName',figName,'defaultDataFolder',defaultDataFolder,'figDir',figDirEach);
end
...
disp(sprintf('[adp_makeFitSimIndividualPlotsNested:fitCRF] Using function: <%s>,  with test: <%s> start...',crfFittype,testType,saveFitFig.testType));
...
switch crfFittype
 ...
 otherwise
  disp(sprintf('[adp_makeFitSimIndividualPlotsNested:fitCRF] %s fit type not defined',crfFittype))
  keyboard
end

disp(sprintf('[adp_makeFitSimIndividualPlotsNested:fitCRF] Using function: <%s>,  with test: <%s> done...',crfFittype,saveFitFig.testType));
...
if ~(length(initparams) == wantedParams)
 disp(sprintf('[adp_makeFitSimIndividualPlotsNested] NOT the right number of parameters - TestType: %s, Number of wanted parameters: %s, Number of parameters: %s', ...
...
% setDataStructure %
%%%%%%%%%%%%%%%%%%%%
function r = setDataStructure(data,meanC, visualArea,analysisType,testAdaptation)
...
if testAdaptation
 for a = 1:length(meanC)
  % make arrays for pedestal contrasts and crfs:
  for j = 1:length(thisContrast)
   r.pedestals(a,j,:)   = meanC{a}.(thisContrast{j})./100;
   
   r.use_crf(a, j,:)     = data{a}.(thisCRF{j});
   r.use_crf_ste(a, j,:) = data{a}.(thisSTE{j});
  end
 end
else
% make arrays for pedestal contrasts and crfs:
